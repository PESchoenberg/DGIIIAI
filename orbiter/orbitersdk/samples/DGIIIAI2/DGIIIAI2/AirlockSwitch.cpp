...
bool AirlockSwitch::Redraw2D (SURFHANDLE surf)
{
	...
	for (i = 0; i < nbutton; i++) 
	{
		...
		{
			case 0: 
				state = (dg->olock_status == DeltaGlider::DOOR_OPEN || dg->olock_status == DeltaGlider::DOOR_OPENING ? 1:0); 
				break;
			case 1: 
				state = (dg->ilock_status == DeltaGlider::DOOR_OPEN || dg->ilock_status == DeltaGlider::DOOR_OPENING ? 1:0); 
				break;
			case 2: 
				state = 0; 
				break;
...
/**
Process airlock switch mouse signals 2D.
*/
bool AirlockSwitch::ProcessMouse2D (int event, int mx, int my)
{
	...
	if (state != btnstate[btn]) 
	{
		...
		{
			case 0: 
				dg->ActivateOuterAirlock (state == 0 ? DeltaGlider::DOOR_CLOSING : DeltaGlider::DOOR_OPENING); 
				return true;
			case 1: 
				dg->ActivateInnerAirlock (state == 0 ? DeltaGlider::DOOR_CLOSING : DeltaGlider::DOOR_OPENING); 
				return true;
			case 2: 
				return false;
		}
